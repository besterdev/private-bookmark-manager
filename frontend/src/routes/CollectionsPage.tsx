...
export default function CollectionsPage() {
  ...
  const load = async () => { setLoading(true); setError(undefined); try { const data = await api.get<Collection[]>('/collections'); setCollections(data); setSelectedId((id) => id && data.some((item) => item.id === id) ? id : data[0]?.id) } catch (cause) { setError({ message: cause instanceof Error ? cause.message : 'Unable to load collections', retry: true }) } finally { setLoading(false) } }
  useEffect(() => { void load() }, [api])
  const create = async (name: string) => { try { const collection = await api.post<Collection>('/collections', { name }); setCollections((items) => [collection, ...items]); setSelectedId(collection.id); setCreateOpen(false) } catch (cause) { setError({ message: cause instanceof Error ? cause.message : 'Unable to create collection', retry: false }) } }
  const remove = async () => { if (!selected) return; try { await api.delete(`/collections/${selected.id}`); setCollections((items) => { const next = items.filter((item) => item.id !== selected.id); setSelectedId(next[0]?.id); return next }); setDeleteOpen(false) } catch (cause) { setError({ message: cause instanceof Error ? cause.message : 'Unable to delete collection', retry: false }) } }
