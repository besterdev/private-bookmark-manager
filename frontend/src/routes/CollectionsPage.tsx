...
export default function CollectionsPage() {
  ...
  const getBookmarks = useCallback(
    (collectionId: string) => api.get<Bookmark[]>(`/collections/${collectionId}/bookmarks`),
    [api],
  )
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setError({ message: 'Unable to load collections', retry: true })
    } finally {
      setLoading(false)
    }
  }, [api])
  ...
  const create = async (name: string) => {
    try {
      const collection = await api.post<Collection>('/collections', { name })
      setCollections((items) => [collection, ...items])
      setSelectedId(collection.id)
      setCreateOpen(false)
    } catch {
      setError({ message: 'Unable to create collection', retry: false })
    }
  }
  const remove = async () => {
    if (!selected) return
    try {
      await api.delete(`/collections/${selected.id}`)
      ...
      setError({ message: 'Unable to delete collection', retry: false })
    }
  }
    ...
    <Stack spacing={3}>
      <Stack direction="row" sx={{ justifyContent: 'space-between' }}>
        <Box>
          ...
          <Typography color="text.secondary">Organize your private bookmarks.</Typography>
        </Box>
      ...
      {collections.length === 0 ? (
        ...
        <Box
          sx={{
            display: 'grid',
            gap: 3,
            gridTemplateColumns: { md: 'minmax(240px, 0.8fr) minmax(0, 1.7fr)' },
          }}
        >
          <CollectionList
      ...
      <Dialog onClose={() => setDeleteOpen(false)} open={deleteOpen}>
        ...
        <DialogContent>This keeps its bookmarks, but makes them uncategorized.</DialogContent>
        <DialogActions>
